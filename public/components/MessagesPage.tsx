...
const MessagesPage: React.FC = () => {
        ...
        const fetchSelectedMessage = async () => {
            if (location.state && location.state.selectedMessageId && location.state.showDetails) {
                try {
                    ...
                    const message = await apiService.getMessageById(location.state.selectedMessageId);
    ...
    const handlePageChange = useCallback((page: number) => {
        ...
        window.scrollTo({
            top: document.querySelector(`.${styles.container}`)?.getBoundingClientRect().top 
                ? window.scrollY + (document.querySelector(`.${styles.container}`)?.getBoundingClientRect().top || 0) - 20
                : 0,
            behavior: 'smooth'
        });
    }, []);
