...
class OpenSearchService {
  private client;
  private index: string = process.env.OPENSEARCH_INDEX || 'messages';
...
public async getMessagesInDateRange(
    ...
    try {
      ...
      console.log(`Found ${response.body.hits.total?.value || 0} messages between ${fromDate} and ${toDate}${direction ? ` with direction '${direction}'` : ''}`);
 ...
 */
public async getMessageChartData(timeframe: 'daily' | 'weekly' | 'monthly', direction?: 'inward' | 'outward') {
 ...
 */
public async getAllMessages(direction?: string): Promise<MessageDocument[]> {
  const now = Date.now();
  ...
  try {
    ...
    console.log(`Found ${response.body.hits.total?.value || 0} messages in OpenSearch`);
 ...
 */
public async getDailyMessages(direction?: string): Promise<MessageDocument[]> {
    ...
    try {
      ...
      console.log(`Found ${response.body.hits.total?.value || 0} messages for today${direction ? ` with direction '${direction}'` : ''}`);
 ...
 */
public async getWeeklyMessages(direction?: string): Promise<MessageDocument[]> {
    ...
    try {
      ...
      console.log(`Found ${response.body.hits.total?.value || 0} messages for this week${direction ? ` with direction '${direction}'` : ''}`);
   ...
   */
  public async getMonthlyMessages(direction?: string): Promise<MessageDocument[]> {
    ...
    try {
      ...
      console.log(`Found ${response.body.hits.total?.value || 0} messages for this month${direction ? ` with direction '${direction}'` : ''}`);
 ...
 */
public async getRecentMessages(limit: number = 5, direction: string = 'All', period: string = 'All'): Promise<{
    id: string;
    time: string;
    mtMessageType: string;
    status: string;
